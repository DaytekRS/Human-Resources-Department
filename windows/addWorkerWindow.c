...
void add_button_worker(GtkWidget *button, gpointer data);
...
void addWorkerWindow(MYSQL* sql){
	...
	gtk_window_set_title(GTK_WINDOW(window), "Добавить работника");
	...
	gtk_table_attach(table,add,1,2,9,10,GTK_FILL,GTK_FILL,0,10);
	
	gtk_container_add (GTK_CONTAINER (window), table);
	gtk_widget_show_all(window);
	g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(gtk_main_quit), NULL);
	gpointer** data=(gpointer**)malloc(sizeof(GtkWidget*)*9+sizeof(char*) + sizeof(MYSQL*) );
	data[0]=eLastname;
	data[1]=eFirstname;
	data[2]=ePatronymic;
	data[3]=ePassport;
	...
	g_signal_connect(GTK_BUTTON(add), "clicked",G_CALLBACK(add_button_worker), data);
...
void add_button_worker(GtkWidget *button, gpointer data){
	...
	sprintf(request,"INSERT INTO users VALUES (NULL,'%s','%s','%s','%s','%s','%s','%s','%s', '%s')",lastName,firstName,patronymic,dateOfBirth,gender,passport,registration,education,place);
	mysql_query(((MYSQL**)data)[8], request);
	free(request);
	free(dateOfBirth);
